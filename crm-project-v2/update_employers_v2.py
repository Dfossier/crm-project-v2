...
# Load all result batches - fix key name issue
all_results = []
for b in range(1, 5):
    with open(f'employer_batch_{b}_results.json') as f:
        ...
        for r in results:
            ...
            employer = r.get('employer') or r.get('employer_found')
            all_results.append({
                'id': r['id'],
                'name': r['name'],
                'employer': employer,
                'source': r.get('source') or r.get('source_url')
            })
...
for r in all_results:
    ...
    # Skip "None" or "null" or "Retired" entries
    if employer and employer not in ['None', 'null', 'Retired', 'Unemployed']:
        cur.execute('UPDATE personnel_990 SET employer = ? WHERE id = ?', (employer, r['id']))
        updated += 1
        print(f'Updated ID {r["id"]}: {r["name"]} -> {employer}')
    elif employer:
        not_found += 1
        print(f'Skipped (retired/none): {r["name"]} - {employer}')
    else:
        not_found += 1
...
print(f'{not_found} records had no employer found or were retired')
